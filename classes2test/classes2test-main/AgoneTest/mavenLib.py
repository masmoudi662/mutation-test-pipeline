...
def run_evosuite_generation_maven(path, focal_path, system):
    ...
    try:
        ...
        if system == 'Windows':
            result_mvn = subprocess.run(['mvn.cmd', 'clean'], cwd=path, capture_output=True, text=True)
            result_generate = subprocess.run(['mvn.cmd', 'evosuite:generate', '-DtimeInMinutesPerClass=1', f'-Dcuts={name_class_to_test}', '-DuseSandbox=false' '-Duse_separate_classloader=false'], cwd=path, capture_output=True, text=True)
            result_export = subprocess.run(['mvn.cmd', 'evosuite:export'], cwd=path, capture_output=True, text=True)
        else:
            result_mvn = subprocess.run(['mvn', 'clean'], cwd=path, capture_output=True, text=True)
            result_generate = subprocess.run(['mvn', 'evosuite:generate', '-DtimeInMinutesPerClass=1', f'-Dcuts={name_class_to_test}', '-DuseSandbox=false' '-Duse_separate_classloader=false'], cwd=path, capture_output=True, text=True)
            result_export = subprocess.run(['mvn', 'evosuite:export'], cwd=path, capture_output=True, text=True)
