...
def conversation(test_type, errors, messages, num_chance):
    """
    Corrects the existing test class based on the provided errors while preserving the specific context for each class.

    Parameters:
        test_type: Type of test to be generated or corrected.
        errors: Description of the compilation errors encountered.
        messages: List of messages to maintain conversation context across correction attempts.
        n (int): The number of correction attempts, used to adjust the guidance provided.

    Returns:
        tuple: A tuple containing:
            - result (str): The corrected and updated test class if the response is valid; otherwise, None.
            - success (bool): True if the correction was successful; False otherwise.
    """

    # Aggiunge il messaggio in base al numero di tentativi
    messages.append(conversation_messages(errors, num_chance))

    # Chiamata API con il contesto specifico
    response = completion(model=test_type, messages=messages, temperature=0)

    if response and response.choices:
        result = response.choices[0].message['content']
        messages.append({"role": "assistant", "content": result})  # Aggiungi la risposta al contesto

        # Rimozione dei delimitatori di codice
        match = re.search(r'```(?:java)?\n(.*?)```', result, re.DOTALL)
        if match:
            result = match.group(1).strip()
        # print_conversation(messages)
        return result, True  # Restituisce la classe corretta e il successo della correzione
